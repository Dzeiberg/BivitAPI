...
//=========================Discussions==================================================
	...
	api.post('/classrooms/:classroomID/articles/:articleID/discussions', function(req, res) {
		Article.findById(req.params.articleID, function(err, article) {
			if (err)
				res.send(err);
			article.discussion.push({"articleIndex" : {"startIDX" : req.body.startIDX, "endIDX" : req.body.endIDX}, "userID" : req.body.userID, "content" : req.body.content});
			article.save(function(err) {
				if (err)
					res.send(err);
...
//THIS METHOD BELOW NEEDS WORK!!

	api.get('/classrooms/:classroomID/articles/:articleID/discussions/', function(req, res) {
		Article.findById(req.params.articleID, function(err, article) {
			if (err)
				res.send(err);
			res.json({message : "article.discussions"});
		});
	});



//article.discussion.push(articleIndex.startIDX : req.body.startIDX, articleIndex.endIDX : req.body.endIDX, content: req.body.DContent,;
	return api;
})();